...
def find_mixed_nn(simi, diffs, test_diff, thre):
    """Find the nearest neighbor using cosine simialrity and bleu score"""
    candidates = simi.argsort()[-thre:][::-1]
    max_score = 0
    max_idx = 0
    for j in candidates:
...
def nngen(train_comments, test_comments):
    counter = TfidfVectorizer()
    train_matrix = counter.fit_transform(train_comments)
    test_matrix = counter.transform(test_comments)
    similarities = cosine_similarity(test_matrix, train_matrix)
    test_nls = []
    for idx, test_simi in enumerate(similarities):
        max_idx = find_mixed_nn(test_simi, train_comments, test_comments[idx], thre=5)
...
for start in tqdm(range(0,len(test_comments), 1000), total=len(test_comments)/1000):
    out_categories.extend(nngen(train_comments, test_comments[start: start + 1000]))
...
df.to_csv('../data/test_ir.csv', index=False)
print(classification_report(test_categories, out_categories, target_names=category_2_label.keys(), digits=5))